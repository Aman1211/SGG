...
function CurrentProjectID() {
	...
	return "{F08CCDDC-FAC2-4A5E-8448-7129975FD6F3}";
}
...
class crChart {
	...
	function SetupChart() {
		...
		if (!ewr_EmptyStr($this->ChartSFldName)) {
			...
			} elseif ($this->Table->TableReportType == "crosstab" && $this->ChartSFldName == $this->Table->ColumnFieldName && $this->Table->ColumnDateSelection && $this->Table->ColumnDateType == "q") { // Quarter
				for ($i = 1; $i <= 4; $i++)
					$this->Series[] = ewr_QuarterName($i);
			} elseif ($this->Table->TableReportType == "crosstab" && $this->ChartSFldName == $this->Table->ColumnFieldName && $this->Table->ColumnDateSelection && $this->Table->ColumnDateType == "m") { // Month
				for ($i = 1; $i <= 12; $i++)
	...
	function Render($width = -1, $height = -1) {
		global $gsExport, $gsCustomExport, $grDashboardReport, $ReportLanguage, $Page, $EWR_NON_FUSIONCHARTS;
...
include_once $EWR_RELATIVE_PATH . "phpmailer605/src/PHPMailer.php";
include_once $EWR_RELATIVE_PATH . "phpmailer605/src/SMTP.php";
include_once $EWR_RELATIVE_PATH . "phpmailer605/src/Exception.php";
