...
<script type="text/javascript">
	$(document).ready(function(){
		$('#b_name').keyup(function(){
				...
				$.post("./check_labour_type.php",{brand_name: brand_check},function(data)
				{
					if(data.status == true)
					{
...
if(isset($_POST['submit']))
{
	...
	$b_name=$_POST['b_name'];
	$qup="select * from labour_type where Labour_type_name='$b_name'";
	$dupquery=mysqli_query($conn,$qup);
	if(mysqli_num_rows($dupquery)>0)
	{
		echo "<script>alert('Labour_Type is already exists.')</script>";
		...
		if($execute)
		{
			...
			//echo "<script>window.open('../index.php?link=$link','_self')</script>";
      		echo"<script>window.open('../view_labour_type.php','_self')</script>"  ;    
       	}   
       	}
       }
	?>
