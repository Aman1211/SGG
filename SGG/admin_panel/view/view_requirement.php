...
$labour="";
$query="select labour from request where Request_id='$require'";
...
if($labour==1)
{
	echo "<script> location.href='./index.php?link=66&edit_r=$require' </script>";
}
 include("./db.php");
      $st="";
	   $abc="select status from request where Request_id='$require'";
...
<div class="search-container">
  ...
  <table id="data">
    ...
    <?php
		 if(isset($_POST['back']))
		 {
			 echo "<script> location.href='./index.php?link=29' </script>";
							}
...
<?php $i++;} ?>
			...
			<?php } else {
			?>
			 <td colspan="8"><button name="view"> VIEW QUOTATION </button> </td>
			<?php } ?>
...
foreach($choices as $key=>$value)
{
	...
	$cnt++;
		...
		$sq_ft=($h_n*$w_n)/144;
		$sq_ft=$sq_ft*$qty;
				$sql="update customer_requirement set Size_Height='$h',Size_Width='$w',rate='$price1',sq_feet='$sq_ft' where Customer_req_id='$req' and Product_id='$pro' and requirement_id='$main'";
...
<script type="text/javascript">
     ...
     $('#submit').val("Inserting");  
						...
						success:function(data){
							...
							$('#req').modal('hide'); 
							location.href="./index.php?link=51&edit_r=<?php echo $require ?>";
    						}
					});
				}
			});
