...
$data = array(
	...
	class mypdf extends fpdf
	{
		...
		function headertable()
		{
			...
			$d=date('d/m/Y');
			$this->Cell(170,10,'DATE-WISE FEEDBACK REPORT',1,0,'C');
			...
			$this->Cell(50,10,'FEEDBACK DESC',1,0,'C');
		...
		function viewtable()
		{
			...
			$newDate1 = date("Y-m-d", strtotime($id2));
			require'./db.php';
			$this->SetFont('Arial','',8);
				//$id=get();
			...
			$f_id="";$i=1;
				...
				$feed = "SELECT * FROM `feedback` WHERE `Feedback_date` BETWEEN '$newDate' AND '$newDate1' order by Feedback_date desc";
				...
				if($feedrun)
				{
					...
					{
						$f_id=$feedrow['Feedback_id'];
						$p_id=$feedrow['Product_id'];
						$desc=$feedrow['Feedback_desc'];
						$date=$feedrow['Feedback_date'];
