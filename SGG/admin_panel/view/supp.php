...
$data = array(
	...
	class mypdf extends fpdf
	{
		...
		function headertable()
		{
			...
			$d=date('d/m/Y');
			$this->Cell(170,10,'SUPPLIER REPORT',1,0,'C');
			...
			$this->Cell(145,10,'PRODUCT NAME',1,0,'C');
			$this->Cell(30,10,'QTY SUPPLIED',1,0,'C');
		...
		function viewtable()
		{
			...
			$f_id="";$i=1;
				...
				$feed= "SELECT  * from `purchase_detail` WHERE `Purchase_id` in(select Purchase_id from purchase where Supplier_id in(select Supplier_id from supplier where Supplier_name='$id'))";
				...
				if($feedrun)
				{
					...
					{
								$f_id=$feedrow['Product_id'];
								$s_id=$feedrow['Qty'];
								$p_name="";
					$query3="select Product_name from product where Product_id='$f_id'";
					...
					while($row3=mysqli_fetch_array($result3))
					{
						...
						$this->Cell(145,10,$p_name,1,0,'C');
							$this->Cell(30,10,$s_id,1,0,'C');
